...
import { UPDATE_MOVIE } from '../graphql/queiries'
...
export default function EditForm({data}) {
    ...
    const formSubmit = async (input) => {
        ...
        try {
            await editMovie({variables: {input, id: data._id}})
            Swal.fire(
                'Edit Movie Success',
                `The movie has been edited from database`,
                'success'
              )
            history.push('/')
        } catch (error) {
    ...
    return (
        ...
        <div className="container w-50 my-4 p-5 d-flex justify-content-center bg-grad-black rad-card form-box"  >
            <div className="w-75">
                <h2 className="text-center mb-4">Edit Movies</h2>
